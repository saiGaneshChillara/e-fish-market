...
app.post('/orders', async (req, res) => {
    ...
    try {
        ...
        const emailId = product.emailId;
        const transporter = nodemailer.createTransport({
            // Set up your email service details (SMTP, etc.)
            service: 'gmail',
            auth: {
                user: 'fishmarket47@gmail.com', // Your email address
                pass: 'iiuebetfvnjylnoe' // Your email password or app-specific password
            }
        });
        const mailOptions = {
            from: 'fishmarket47@gmail.com', // Sender's email address
            to: emailId, // Receiver's email address (taken from the product details)
            subject: 'You got a new order!!',
            text: `${firstname} ${lastname} has ordered ${product.productname} of quantity ${quantity} to delvier at address ${address}` // Add your email message content
        };
...
app.post('/login', async (req, res) => {
    ...
    const isAdmin = email == 'aman@gmail.com' && password == 'aman@19';
...
app.post('/register', async (req, res) => {
    try {
        ...
        res.status(201).json({newUser: userCreated, message:'Successfully Registered'});
    } catch (error) {
        console.log(error);
